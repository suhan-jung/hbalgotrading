...
class StrategyBuyAndHold(Strategy):
    ...
    MULTIPLIER = 1000000 # 국채선물 거래승수 : 100만, 나중에 대상자산별로 구조화 필요
    ROUNDDOWN = 0 # 계약수(amount) rounddown 시 상수, 장내선물 소숫점 거래 불가
    ...
    def update_result(self, result):
        ...
        try:
            ...
            total = float(result["price"]) * float(result["amount"]) * self.MULTIPLIER
            # fee = total * self.COMMISSION_RATIO
            fee = float(result["amount"]) * 800
            if result["type"] == "buy":
    ...
    def get_request(self):
        ...
        try:
            ...
            # amount = round(target_budget / last_closing_price, 4) # 암호화폐용이라서 소수점 넷째자리까지 round. 수정 필요
            amount = round(target_budget / last_closing_price / self.MULTIPLIER, self.ROUNDDOWN)
            trading_request = {
            ...
            # total_value = round(float(last_closing_price) * amount) # 국채선물로 변경시 multiplier 필요
            total_value = round(float(last_closing_price) * amount * self.MULTIPLIER)
